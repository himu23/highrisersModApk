...
public abstract class CodedInputStream {
    ...
    private static final class UnsafeDirectNioDecoder extends CodedInputStream {
        ...
        public int readRawLittleEndian32() throws IOException {
            ...
            if (this.limit - j >= 4) {
                this.pos = 4 + j;
                return ((UnsafeUtil.getByte(j + 3) & UByte.MAX_VALUE) << Ascii.CAN) | (UnsafeUtil.getByte(j) & UByte.MAX_VALUE) | ((UnsafeUtil.getByte(1 + j) & UByte.MAX_VALUE) << 8) | ((UnsafeUtil.getByte(2 + j) & UByte.MAX_VALUE) << Ascii.DLE);
            }
            throw InvalidProtocolBufferException.truncatedMessage();
        }
    ...
    private static final class IterableDirectByteBufferDecoder extends CodedInputStream {
        ...
        public int readRawLittleEndian32() throws IOException {
            if (currentRemaining() < 4) {
                return (readRawByte() & UByte.MAX_VALUE) | ((readRawByte() & UByte.MAX_VALUE) << 8) | ((readRawByte() & UByte.MAX_VALUE) << Ascii.DLE) | ((readRawByte() & UByte.MAX_VALUE) << Ascii.CAN);
            }
            long j = this.currentByteBufferPos;
