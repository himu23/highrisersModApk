...
public class Intent {
    ...
    public static void launch(JSONObject jSONObject, WebViewCallback webViewCallback) {
        ...
        String str2 = (String) jSONObject.opt(HandleAndroidInvocationsUseCase.KEY_PACKAGE_NAME);
        String str3 = (String) jSONObject.opt(m2.h.h);
    ...
    private static boolean setExtras(android.content.Intent intent, JSONArray jSONArray) {
        ...
        while (i < jSONArray.length()) {
            try {
                ...
                if (!setExtra(intent, jSONObject.getString(m2.h.W), jSONObject.get(m2.h.X))) {
                    return false;
                }
                i++;
            } catch (Exception e) {
    ...
    public static void canOpenIntents(JSONArray jSONArray, WebViewCallback webViewCallback) {
        ...
        while (i < length) {
            ...
            try {
                jSONObject.put(optJSONObject.optString(w5.x), checkIntentResolvable(intentFromMetadata(optJSONObject)));
                i++;
            } catch (IntentException e) {
